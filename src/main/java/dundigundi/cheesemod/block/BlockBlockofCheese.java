...
public class BlockBlockofCheese extends BlockCake {
	...
	public ItemStack[] getBreakResult(World world, EnumDropCause dropCause, int x, int y, int z, int meta, TileEntity tileEntity) {
		return dropCause != EnumDropCause.PICK_BLOCK && meta != 0 ? null : new ItemStack[]{new ItemStack(CheeseModBlocks.blockOfCheese)};
	}
	@Override
	...
	private void eatCakeSlice(World world, int i, int j, int k, EntityPlayer entityplayer) {
		if (entityplayer.inventory.getCurrentItem() != null){
			if(entityplayer.inventory.getCurrentItem().itemID == CheeseModItems.knife.id){
				entityplayer.inventory.insertItem(new ItemStack(CheeseModItems.cheeseSlice),false);
	...
	public int getBlockTextureFromSideAndMetadata(Side side, int data) {
		return data > 0 && side == Side.WEST ? TextureHelper.getOrCreateBlockTextureIndex(CheeseMod.MOD_ID, "blockOfCheeseSide.png") : this.atlasIndices[side.getId()];
	}
}
